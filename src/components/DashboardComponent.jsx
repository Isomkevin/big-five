...
import { BellIcon, ChevronDownIcon, Bars3Icon, MagnifyingGlassIcon, UserCircleIcon, CreditCardIcon, CurrencyDollarIcon, UsersIcon, } from '@heroicons/react/24/outline';
import { Avatar, Badge, Box, Button, Input, Table, Tbody, Td, Th, Thead, Tr, } from '@chakra-ui/react';
    ...
    <div className="flex min-h-screen w-full flex-col">
      <header className="sticky top-0 flex h-16 items-center gap-4 border-b bg-white px-4 md:px-6">
        <nav className="hidden flex-col gap-6 text-lg font-medium md:flex md:flex-row md:items-center md:gap-5 md:text-sm lg:gap-6">
          ...
          <Link to="#" className="text-gray-700 transition-colors hover:text-gray-900">
            Dashboard
          </Link>
        ...
        <Menu as="div" className="relative md:hidden">
          <MenuButton className="shrink-0 p-2 rounded-md border border-gray-300">
          ...
          >
            <MenuItems className="absolute left-0 mt-2 w-48 origin-top-left rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
              <div className="py-1">
                <MenuItem>
                  {({ focus }) => (
                    <Link to="#" className={`flex items-center gap-2 px-4 py-2 text-lg font-semibold ${focus ? 'bg-gray-100' : ''}`}>
                      <CurrencyDollarIcon className="h-6 w-6" />
                      <span className="sr-only">Acme Inc</span>
                    </Link>
                  )}
                ...
                <MenuItem>
                  {({ focus }) => (
                    <Link to="#" className={`block px-4 py-2 text-lg ${focus ? 'bg-gray-100' : ''}`}>
                      Dashboard
                    </Link>
                  )}
                </MenuItem>
            ...
            >
              <MenuItems className="absolute right-0 mt-2 w-48 origin-top-right rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div className="py-1">
                  <MenuItem>
                    {({ focus }) => (
                      <a className={`block px-4 py-2 text-sm ${focus ? 'bg-gray-100' : ''}`}>
                        My Account
                      </a>
                    )}
                  </MenuItem>
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        ...
        <div>
          <Table className="w-full divide-y divide-gray-200 bg-white shadow rounded-lg">
            ...
            <Tbody>
              {[
                { name: 'John Doe', title: 'CEO', status: 'Active', role: 'Admin', email: 'john@example.com' },
                { name: 'Jane Smith', title: 'Designer', status: 'Inactive', role: 'Member', email: 'jane@example.com' },
                { name: 'Sam Green', title: 'Developer', status: 'Active', role: 'Member', email: 'sam@example.com' },
              ].map((user, index) => (
                <Tr key={index}>
